chrome.storage.sync.get({}, items => {
    setTimeout(elementRandomizer, delay);
    ...
    function elementRandomizer() {
        ...
        if (getEnabled("randomClassList")) {
            everything.forEach(element => {
                const randomClassList = everything[Math.floor(Math.random() * everything.length)].classList;
                element.classList = randomClassList;
            });
        }

        if (getEnabled("randomContentEditable")) {
            everything.forEach(element => element.contentEditable = booleans[Math.floor(Math.random() * booleans.length)]);
        }

        if (getEnabled("randomDisabled")) {
        ...
        if (getEnabled("randomDraggable")) {
            everything.forEach(element => element.draggable = booleans[Math.floor(Math.random() * booleans.length)]);
        }

        if (getEnabled("randomID")) {
            everything.forEach(element => {
                const randomID = everything[Math.floor(Math.random() * everything.length)].id;
                element.id = randomID;
            });
        }

        if (getEnabled("randomListReversed")) {
        ...
        if (getEnabled("randomMaxLength")) {
            inputs.forEach(input => input.maxLength = getRandomNumber(1, 50));
        }

        if (getEnabled("randomOptionSelected")) {
        ...
        if (getEnabled("randomTabIndex")) {
            everything.forEach(element => element.tabIndex = getRandomNumber(0, everything.length));
        }

        if (getEnabled("randomTooltip")) {
            everything.forEach(element => element.title = getRandomText());
        }

        if (getEnabled("randomType")) {
            inputs.forEach(input => input.type = types[Math.floor(Math.random() * types.length)]);
        }

        if (getEnabled("randomBackgroundColor")) {
            randomStyle("backgroundColor", () => `rgb(${getRandomNumber(0, 255)}, ${getRandomNumber(0, 255)}, ${getRandomNumber(0, 255)})`);
        }

        if (getEnabled("randomBackgroundImage")) {
            randomStyle("background", () => {
        ...
        if (getEnabled("randomCornerRadius")) {
            randomStyle("borderRadius", () => `${getRandomNumber(0, 100)}px ${getRandomNumber(0, 100)}px ${getRandomNumber(0, 100)}px ${getRandomNumber(0, 100)}px`);
        }

        if (getEnabled("randomBoxShadow")) {
            randomStyle("boxShadow", () => {
        ...
        if (getEnabled("randomCursor")) {
            randomStyle("cursor", () => cursors[Math.floor(Math.random() * cursors.length)]);
        }

        if (getEnabled("randomDisplayType")) {
            randomStyle("display", () => displayValues[Math.floor(Math.random() * displayValues.length)]);
        }

        if (getEnabled("randomFloat")) {
            randomStyle("float", () => floatValues[Math.floor(Math.random() * floatValues.length)]);
        }

        if (getEnabled("randomHeight")) {
            randomStyle("height", () => `${getRandomNumber(0, 100)}%`);
        }

        if (getEnabled("randomMove")) {
            randomStyle("transform", () => `translate(${getRandomNumber(-200, 200)}px, ${getRandomNumber(-200, 200)}px)`);
        }

        if (getEnabled("randomOpacity")) {
            randomStyle("opacity", () => Math.random());
        }

        if (getEnabled("randomOutline")) {
            randomStyle("outline", () => {
                const randomThickness = getRandomNumber(1, 10);
                const randomStyle = borderStyles[Math.floor(Math.random() * borderStyles.length)];
                const randomColor = `rgb(${getRandomNumber(0, 255)}, ${getRandomNumber(0, 255)}, ${getRandomNumber(0, 255)})`;
        ...
        if (getEnabled("randomOverflow")) {
            randomStyle("overflow", () => overflowValues[Math.floor(Math.random() * overflowValues.length)]);
        }

        if (getEnabled("randomPadding")) {
            randomStyle("padding", () => `${getRandomNumber(0, 50)}px ${getRandomNumber(0, 50)}px ${getRandomNumber(0, 50)}px ${getRandomNumber(0, 50)}px`);
        }

        if (getEnabled("randomPositioningType")) {
            randomStyle("position", () => positionValues[Math.floor(Math.random() * positionValues.length)]);
        }

        if (getEnabled("randomResizeAbility")) {
            randomStyle("resize", () => resizeValues[Math.floor(Math.random() * resizeValues.length)]);
        }

        if (getEnabled("randomRotation")) {
            randomStyle("transform", () => `rotate(${getRandomNumber(-360, 360)}deg)`);
        }

        if (getEnabled("randomStackOrder")) {
            randomStyle("zIndex", () => getRandomNumber(-1, 5));
        }

        if (getEnabled("randomVisibility")) {
            randomStyle("visibility", () => visibilityValues[Math.floor(Math.random() * visibilityValues.length)]);
        }

        if (getEnabled("randomWidth")) {
            randomStyle("width", () => `${getRandomNumber(0, 100)}%`);
        }
    }
});